...
class UserController < ApplicationController
    ...
    def list_user
        ...
        elsif params[:user_type] == 'Zaměstnanec'
            render(:update) do |page|
        ...
        if request.post?
            ...
            if User.authenticate?(@user.username, params[:user][:old_password])
                if params[:user][:new_password] == params[:user][:confirm_password]
                    ...
                    flash[:warn_notice] = '<p>Potvrzení hesla selhalo. Prosím opakujte akci.</p>'
                end
            else
                flash[:warn_notice] = '<p>Staré heslo, které jste uvedli je neplatné. Prosím uveďte správné heslo.</p>'
            end
        end
    end
    ...
    def user_change_password
        ...
        if request.post?
            if params[:user][:new_password]=='' and params[:user][:confirm_password]==''
                flash[:warn_notice]= "<p>Pole hesla nemůže být prázdné!</p>"
            else
                if params[:user][:new_password] == params[:user][:confirm_password]
                    user.password = params[:user][:new_password]
                    user.update_attributes(:password => user.password,
                    ...
                    flash[:notice]= "Heslo bylo úspěšně aktualizováno!"
                    redirect_to :action=>"edit", :id=>user.username
                else
                    flash[:warn_notice] = '<p>Potvrzení hesla je neplatné. Prosím opakujte akci.</p>'
                end
            end
        ...
        if request.post?
            ...
            if @config.include?('HR')
                ...
                if @employee.save and @user.save
                    flash[:notice] = 'Uživatelský účet vytvořen!'
                    redirect_to :controller => 'user', :action => 'edit', :id => @user.username
                else
                    flash[:notice] = 'Uživatelský účet byl vytvořen!'
                end
            end
        end
    end
    ...
    def delete
        ...
        flash[:notice] = 'Uživatelský účet smazán!'
    ...
    def edit
        ...
        if request.post? and @user.update_attributes(params[:user])
            flash[:notice] = 'Uživatelský účet aktualizován!'
            redirect_to :controller => 'user', :action => 'profile', :id => @user.username
        end
    end

    def forgot_password
        #    flash[:notice]="Nemáte oprávnění pro přístup k zapomenutému heslu!"
        ...
        if request.post? and params[:reset_password]
            if user = User.find_by_email(params[:reset_password][:email])
                ...
                flash[:notice] = "Resetovat odkaz Hesla odeslán emailem #{user.email}"
                redirect_to :action => "index"
            else
                flash[:notice] = "Neexistuje žádný uživatel s uvedenou emailovou adresou #{params[:reset_password][:email]}"
            end
        end
    end
    ...
    def login
        ...
        if request.post? and params[:user]
            ...
            if user and User.authenticate?(@user.username, @user.password)
                ...
                flash[:notice] = "Vítejte, #{user.first_name} #{user.last_name}!"
                redirect_to :controller => 'user', :action => 'dashboard'
            else
                flash[:notice] = 'Neplatné uživateslké jméno nebo heslo'
            end
        end
    end
    ...
    def profile
        ...
        unless @user.nil?
            ...
            flash[:notice] = 'Uživatelský účet nenalezen.'
            redirect_to :action => 'dashboard'
        end
    end
    ...
    def reset_password
        ...
        if user
            if user.reset_password_code_until > Time.now
                ...
                flash[:notice] = 'Expirován čas resetu'
                redirect_to :action => 'index'
            end
        else
            flash[:notice]= 'Neplatný odkaz resetu'
            redirect_to :action => 'index'
        end
    end
        ...
        if request.post?
            ...
            if user
                if params[:set_new_password][:new_password] === params[:set_new_password][:confirm_password]
                    ...
                    flash[:notice] = 'Heslo úspěšně obnoveno. Použíjte nové heslo pro přihlášení.'
                    redirect_to :action => 'index'
                else
                    flash[:notice] = 'Potrvzení hesla selhalo. Prosím zadejte znovu.'
                    redirect_to :action => 'set_new_password', :id => user.reset_password_code
                end
            else
                flash[:notice] = 'Zadaný odkaz je neplatný. Prosím zkuste znovu.'
                redirect_to :action => 'index'
            end
        end
    end
