...
class ExamController < ApplicationController
    ...
    def update_exam_form
        ...
        unless @name == ''
            ...
            render(:update) do |page|
                page.replace_html 'flash', :text=>'<div class="errorExplanation"><p>Název Hodnocení nemůže být prázdné</p></div>'
            end
        end
    end
    ...
    def publish
        ...
        if params[:status] == "schedule"
            ...
            students.each do |s|
                ...
                Reminder.create(:sender=> current_user.id,:recipient=>student_user.id,
                    :subject=>"Plánované hodnocení",
                    :body=>"#{@exam_group.name} byla naplánovaná  <br/> Prosím zobrazte kalendář pro více podrobností")
            end
        ...
        unless @exams.empty?
            ...
            @no_exam_notice = "Hodnocení nebylo dosud naplánováno."
        end
    end
    ...
    def grouping
        ...
        if request.post?
            ...
            flash[:notice]="Vybrané hodnocení úspěšně spojeno."
        end
    end
    ...
    def generated_report
        if params[:student].nil?
            if params[:exam_report].nil? or params[:exam_report][:exam_group_id].empty?
                flash[:notice] = "Vyberte třídu a hodnocení pro pokračování."
                redirect_to :action=>'exam_wise_report' and return
            end
        else
            if params[:exam_group].nil?
                flash[:notice] = "Neplatné údaje."
                redirect_to :action=>'exam_wise_report' and return
            end
        end
        if params[:student].nil?
...
#        end
    ...
    def generated_report2
        ...
        unless params[:exam_report][:subject_id] == ""
            ...
            flash[:notice] = "vyberte skupinu pro pokračování"
...
#        end
    ...
    def generated_report4
        ...
        if params[:student].nil?
            ...
            @student = @batch.students.first
            if @student.blank?
                flash[:notice] = "Žádný hráč není ve vybrané třídě"
                redirect_to :action=>'grouped_exam_report' and return
            end
            if @type == 'grouped'
                @grouped_exams = GroupedExam.find_all_by_batch_id(@batch.id)
