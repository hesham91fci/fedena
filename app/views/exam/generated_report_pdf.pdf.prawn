...
@students.each do |student|
  ...
  if @exam_group.exam_type == 'Marks'
    data.push ["Subject","Marks Obtained","Maximum marks","Percentage(%)"]
    total_marks = 0
    total_max_marks = 0
    ...
    data.push ["total",total_marks,total_max_marks,(@total_percentage)]
    ...
    @exam_group.exams.each do |exam|
      exam_score = ExamScore.find_by_student_id_and_exam_id(student,exam)
      unless exam_score.nil?
        data.push [exam.subject.name,exam_score.grading_level || "-"]
      else
    ...
    data.push ["Subject","Grade","Marks Obtained","Maximum marks","Percentage(%)"]
    total_marks = 0
    total_max_marks = 0
    @exam_group.exams.each do |exam|
      exam_score = ExamScore.find_by_student_id_and_exam_id(student,exam)
      ...
      data.push [exam.subject.name,grade,mark,exam.maximum_marks,(exam_score.calculate_percentage unless exam_score.nil?)]
    end
    data.push ["total","-",total_marks,total_max_marks,(@total_percentage)]
    pdf.table data, :width => 500,
      :border_color => "000000",
  ...
  pdf.start_new_page unless student == @students.last
end
